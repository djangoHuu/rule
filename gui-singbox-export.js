    ...
    ;[...profile.dnsRulesConfig, ...profile.rulesConfig].forEach((rule) => {
      if (rule.type === 'rule_set') {
        ...
        if (rule.payload.startsWith('geosite_') || rule.payload.startsWith('geoip_')) {
          ...
          rule.payload = rule.payload.replace('geosite_', 'https://github.com/MetaCubeX/meta-rules-dat@sing/geo-lite/geosite/')
          rule.payload = rule.payload.replace('geoip_', 'https://github.com/MetaCubeX/meta-rules-dat@sing/geo-lite/geoip/')
